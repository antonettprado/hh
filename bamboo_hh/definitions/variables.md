...
- `hists`: Dictionary storing histograms associated with this variable, identified by unique keys. Default: `{}` until a histogram(s) is(are) generated. The key for a particular hist is `{subcat}_{varname}_{sample}`, ex. `SL_res_4j_2b_bjets_mbb_bbWW_sl`.
