...
class SL_DL_vars_reco(SL_DL_event_selection):
    ...
    def additional_event_selection(self, tree, baseSel, yields):
        self.tree = tree
        ...
        selections =  {'SL_res_1b':  SL_res_1b,   'SL_res_2b':  SL_res_2b,   'SL_boost':SL_boost, 
                      ...
                      'SL_res_1b_x':SL_res_1b_x, 'SL_res_2b_x':SL_res_2b_x, 'baseSel':baseSel}
    ...
    def get_skims(self, vars: 'list[Variable]', plots):
        for selection_name, selection in self.selections.items():
            ...
            values = [sub_var.ref for var in vars for sub_var in var if sub_var.subcat == selection_name]
            branches = dict(zip(keys, values))
            branches.update({"event":None})
            # branches.update({"Electron_pt":None})
            # branches_from_input_tree = ["event", "Electron_pt"]
    ...
    def definePlots(self, tree, baseSel, sample=None, sampleCfg=None):
        ...
        self.selections = super().event_selection(tree, baseSel, self.objects, yields, events='even')
        self.selections = self.additional_event_selection(tree, baseSel, yields)
        ...
        # plots = self.get_skims(reco_vars + reco_2D_vars, plots)
