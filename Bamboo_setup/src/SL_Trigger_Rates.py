...
class SL_Trigger_Rates(NanoAODHistoModule):
    ...
    Triggers = [
        ...
        'HLT_PPSMaxTracksPerArm1',
        'HLT_PPSMaxTracksPerRP4',
        'HLT_PPSRandom',
        'HLT_Photon100EBHE10',
        ...
        'HLT_ZeroBias',                                 # Ignoring all HLT_ZeroBias ones
    ...
    def definePlots(self, tree, noSel, sample=None, sampleCfg=None):
        ...
        sel = noSel.refine('total_rate', cut=op.AND(137<=tree.luminosityBlock, tree.luminosityBlock <= 591, tree.run==380963))

        plots.append(Skim('total_rate', {"Mu12": tree.HLT.Mu12_IsoVVL_PFHT150_PNetBTag0p53, "Ele14": tree.HLT.Ele14_eta2p5_IsoVVVL_Gsf_PFHT200_PNetBTag0p53}, sel))

        all_other_triggers = []
        for trig_name in SL_Trigger_Rates.Triggers:
            trig_name = trig_name.removeprefix('HLT_')
            if trig_name not in ['Mu12_IsoVVL_PFHT150_PNetBTag0p53', 'Ele14_eta2p5_IsoVVVL_Gsf_PFHT200_PNetBTag0p53']:
                all_other_triggers.append(getattr(tree.HLT, trig_name))

        sel_Mu = sel.refine('pure_rate_Mu', cut=op.AND(tree.HLT.Mu12_IsoVVL_PFHT150_PNetBTag0p53, *[op.NOT(trigger) for trigger in all_other_triggers]))     
        sel_Ele = sel.refine('pure_rate_Ele', cut=op.AND(tree.HLT.Ele14_eta2p5_IsoVVVL_Gsf_PFHT200_PNetBTag0p53, *[op.NOT(trigger) for trigger in all_other_triggers]))       

        plots.append(Skim('sel_Mu', {"Mu12": tree.HLT.Mu12_IsoVVL_PFHT150_PNetBTag0p53}, sel_Mu))
        plots.append(Skim('sel_Ele', {"Ele14": tree.HLT.Ele14_eta2p5_IsoVVVL_Gsf_PFHT200_PNetBTag0p53}, sel_Ele))
    ...
    def postProcess(self, taskList, config=None, workdir=None, resultsdir=None):
        ...
        if 'Muon_Run2024E' in config['samples'].keys():
            upfile = uproot.open(Path(resultsdir)/'Muon_Run2024E.root')
            df_tr = upfile['total_rate'].arrays(library='pd')
            HLT_Mu12_firings = df_tr['Mu12'].sum()

            print('Total Rates:')
            print(f"HLT_Mu12_firings: {HLT_Mu12_firings}")
            print(f"HLT_Mu12: {HLT_Mu12_firings / time}")

            df_sel_Mu = upfile['sel_Mu'].arrays(library='pd')
            Mu_events = len(df_sel_Mu)
            print('\nPure Rates:')
            print(f"HLT_Mu12 standalone firings: {Mu_events}")
            print(f"HLT_Mu12: {Mu_events / time}")
            
        
        elif 'EGamma_Run2024E' in config['samples'].keys():
            upfile = uproot.open(Path(resultsdir)/'EGamma_Run2024E.root')
            df_tr = upfile['total_rate'].arrays(library='pd') 
            HLT_Ele14_firings = df_tr['Ele14'].sum()

            print('Total Rates:')
            print(f"HLT_Ele14_firings: {HLT_Ele14_firings}")
            print(f"HLT_Ele14: {HLT_Ele14_firings / time}")

            df_sel_Ele = upfile['sel_Ele'].arrays(library='pd')
            Ele_events = len(df_sel_Ele)
            print('\nPure Rates:')
            print(f"HLT_Ele14 standalone firings: {Ele_events}")
            print(f"HLT_Ele14: {Ele_events / time}")
