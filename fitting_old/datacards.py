...
class Datacard():
    ...
    def from_combination(cls, datacards: list[Self], path: Path) -> Self:
        ...
        selection = "_".join(list(dict.fromkeys([dc.selection for dc in datacards])))  
        ...
        era = "_".join(list(dict.fromkeys([dc.era for dc in datacards])))  
...
def parse_as_DNN_model(split_hist_names: list[list[str]], eras: set[str]) -> tuple[list[Datacard], list[int]]:
    ...
    for i, (selection, category, model, variable) in enumerate(split_hist_names):
        for era in eras:
...
def parse_results_files(results_dir: Path) -> tuple[list[Datacard], list[str]]:
    ...
    hist_names = list(filter(lambda name: name.count('_x_') == 2, hist_names))

    def process_name(name):
        # Replace '_xxx_' with '_x_' to treat it as a separator
        name = name.replace('_xxx_', '_x_')
        # Then split on '_x_'
        parts = name.split('_x_')
        return parts

    for name in hist_names:
        print(f"{name:<60}", process_name(name))

    split_hist_names = [process_name(name) for name in hist_names]
    datacards, hist_indices_for_fit = parse_as_DNN_model(split_hist_names, eras)
