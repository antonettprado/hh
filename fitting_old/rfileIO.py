...
from core.analysis_config import parseAnalysisConfig
...
def combine_histos_from_root_files(root_files: Iterable[Path], process_map: dict[str, str], xs: dict[str, str], lumi: float, hist_names: list[str]=None):
    ...
    for f in root_files:
        era = functions.get_file_era(f)
        subprocess = functions.get_file_subprocess(f)
...
def combine_results(results_dir: Path, hist_names: list[str]=None) -> dict:
    files: list[Path] = functions.get_root_files(results_dir)
    eras: list[str] = functions.get_eras(results_dir)
    ...
    process_map = { sub: process
                    for process, sub_processes in constants.PROCESSES_FILES.items()
