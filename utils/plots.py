...
def hist1d_to_numpy(hist1d):
    values = np.array([hist1d.GetBinContent(i + 1) for i in range(hist1d.GetNbinsX())])
    edges = np.array([hist1d.GetBinLowEdge(i + 1) for i in range(hist1d.GetNbinsX() + 1)])
    return values, edges
...
def hist2d_to_numpy(hist2d):
    ...
    for i in range(nx):
        for j in range(ny):
            values[j, i] = hist2d.GetBinContent(i + 1, j + 1)
    x_edges = np.array([hist2d.GetXaxis().GetBinLowEdge(i + 1) for i in range(nx + 1)])
    y_edges = np.array([hist2d.GetYaxis().GetBinLowEdge(i + 1) for i in range(ny + 1)])
    return values, x_edges, y_edges

def hist3d_to_numpy(hist3d):
    """Convert ROOT TH3 histogram to numpy arrays"""
    nx = hist3d.GetNbinsX()
    ny = hist3d.GetNbinsY()
    nz = hist3d.GetNbinsZ()
    
    values = np.zeros((nz, ny, nx))  # Note: z, y, x order for numpy
    for i in range(nx):
        for j in range(ny):
            for k in range(nz):
                values[k, j, i] = hist3d.GetBinContent(i + 1, j + 1, k + 1)
    
    x_edges = np.array([hist3d.GetXaxis().GetBinLowEdge(i + 1) for i in range(nx + 1)])
    y_edges = np.array([hist3d.GetYaxis().GetBinLowEdge(i + 1) for i in range(ny + 1)])
    z_edges = np.array([hist3d.GetZaxis().GetBinLowEdge(i + 1) for i in range(nz + 1)])
    
    return values, x_edges, y_edges, z_edges
    
def plot_1d(values: list, edges: list, labels: list[str], colors: list[str], fill: bool,
    ...
    for i in range(len(values)):
        ...
        label_i = labels[i]
        edges_i = edges[i]
        ax.step(edges_i[:-1], values_i, where='post', color=color_i, linewidth=2.0, label=label_i)
        if fill:
