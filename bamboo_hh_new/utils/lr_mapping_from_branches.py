...
from bamboo_hh_new.utils.histograms import normalize_hist, get_total_hist_from_branches
...
def build_correction_from_var(args):
    var, signal_files, background_files, tree_name, config, take_log = args
    try:
        signal_hist = get_total_hist_from_branches(signal_files, tree_name, var, config)
        background_hist = get_total_hist_from_branches(background_files, tree_name, var, config)
...
def main(workdir: Path, take_log: bool=False, outfilename: str = None):
    ...
    for tree_name in selections:
        ...
        present_vars1D = REG.get_present_vars('1D', signal_files[0], tree_name=tree_name)
        task_args = [
            (var, signal_files, background_files, tree_name, config, take_log)
