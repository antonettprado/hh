...
from bamboo_hh_new.utils.histograms import normalize_hist, get_hist_refs_from_file, get_total_hist_from_histograms
...
def main(workdir: Path, take_log: bool=False, outfilename: str = None):
    ...
    for sel_name in selections:
        ...
        for ref in sel_refs:
            ...
            var_name = ref.removeprefix(f"{sel_name}_")
            if var_name in present_vars1D:
            ...
            corr = cs.Correction(
                ...
                data=data)
            print(f"Correction created for {ref}: {var_name=} ...")
