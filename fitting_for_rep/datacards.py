...
from fitting_for_rep import rfileIO
...
def generate_datacard_text(rfile_path: Path, process_rates: dict[str, float], obs_process: str, dc: Datacard, signal: str='HH_bbWW') -> str:
...
def underscore_split(hist_names: list[str]) -> list[list[str]]:
    ...
    for n in hist_names:
        split = n.split('_')
        selection = '_'.join(split[:3])
        model = '_'.join(split[3:8])
        variable = split[8]
        cat = split[9] if len(split) > 9 else 'whole'
        recomb = (selection, f'{variable}_{cat}', variable, model)
        if n.startswith('SL_res_2b_x_'):
            recomb = ('_'.join(split[:4]), '_'.join(split[4:6]), split[6], '_'.join(split[7:]))
        split_hist_names.append(recomb)
        print(f'Splitting {n} into {recomb}')
...
def main() -> None:
    args = parse_args()
    dcs = make_datacards(nndir=args.nndir, results_dir=args.input)
